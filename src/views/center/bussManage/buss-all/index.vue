...
export default {
    components: { SwitchForm, distribution },
    data() {
        return {
            //显示搜索框
            showSearch: true,
    ...
    methods: {
        ...
        getList() {   //获取table表单的数据**************************************
            ...
            bussAllList(this.addDateRange(this.queryParams, this.dateRange)).then(response => {
        ...
        seleceUserFinish(userObj) {
            bussDistribution({ oppIds: this.ids, disTraUserId: userObj.id }).then(res => {

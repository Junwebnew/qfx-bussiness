    ...
    methods: {
        ...
        uploadImg() {
            this.$refs.cropper.getCropBlob(data => {
                ...
                const newFile = new window.File([data], new Date().getTime() + '.jpg', { type: 'jpg' });

                let formData = new FormData();

                formData.append("fileSource", 'ALIYUN');
                ...
                fileUpload(formData).then(response => {
                    imgId = response.id
                    return updateLoginUserInfo({ headImg: response.id })

                }).then(res => {

                    this.open = false;
                    // this.options.img = process.env.VUE_APP_BASE_API + response.imgUrl;
                    store.commit('SET_AVATAR', this.$getImg(imgId, 1));
